...
class TestTaskModel:
    ...
    async def test_create_task(self, user):
        ...
        assert task.callback_id == "test_callback_id"
        await task.delete()
    ...
    async def test_task_to_dict(self, user):
        ...
        assert "callback_id" in task_dict
        await task.delete()
...
class TestTaskAPI:
    ...
    async def test_create_task_api(self, client, headers, mocker):
        ...
        mocker.patch(
            "scheduler_service.service.request.get_session", return_value=mock_client
        )
    ...
    async def test_bulk_create_task(self, client, headers, mocker):
        ...
        resp = await client.post(
            f"{const.TASK_URL}/bulk", headers=headers, json=tasks_data
        )
        assert resp.status_code == 200
        response_data = resp.json()
    ...
    async def test_user_isolation(self, client, headers, user):
        ...
        resp = await client.post(
            const.AUTH_TOKEN_URL, json={"name": "user2", "password": "password"}
        )
...
class TestDramatiqActors:
    ...
    async def test_ping_actor_success(self, stub_broker, stub_worker):  # 恢复fixture
        ...
        mock_task.callback_id = "test_callback_123"
        mock_task.callback_token = "valid_token_123"
        ...
        with patch(
            ...
            with patch(
                ...
                if mock_task.callback_id:
                    expected_callback_data["callback_id"] = mock_task.callback_id

                expected_headers = {}
                if mock_task.callback_token:
                    expected_headers["Authorization"] = (
                        f"Bearer {mock_task.callback_token}"
                    )

                mock_session.post.assert_called_once_with(
                    mock_task.callback_url,
                    json=expected_callback_data,
    ...
    async def test_ping_actor_methods(self, method, stub_broker, stub_worker):
        ...
        mock_task.body = (
            {"data": "test"} if method in ["POST", "PUT", "PATCH"] else None
        )
